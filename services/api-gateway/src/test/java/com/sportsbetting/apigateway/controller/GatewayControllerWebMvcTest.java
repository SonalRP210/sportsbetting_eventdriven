...
class GatewayControllerWebMvcTest {
    ...
    void allRoutesDelegateToService() throws Exception {
        ...
        when(gatewayService.login(anyString())).thenReturn(
                ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED)
                        .body("{\"error\":\"AUTH_NOT_IMPLEMENTED\"}"));
        when(gatewayService.getUser(eq("u1"), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("user"));
        when(gatewayService.upsertUser(anyString(), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("ok"));
        when(gatewayService.ingestProviderEvent(anyString(), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("ok"));
        when(gatewayService.placeBet(anyString(), nullable(String.class), any(DownstreamAuthHeaders.class)))
                .thenReturn(ResponseEntity.status(201).body("bet"));
        when(gatewayService.getBet(eq("b1"), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("bet"));
        when(gatewayService.cancelBet(eq("b1"), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("canceled"));
        when(gatewayService.oddsFeed(anyString(), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("feed"));
        when(gatewayService.settleEvent(anyString(), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("settled"));
        when(gatewayService.userBets(eq("u1"), anyInt(), anyInt(), any(DownstreamAuthHeaders.class)))
                .thenReturn(ResponseEntity.ok("[]"));
        when(gatewayService.eventBets(eq("e1"), anyInt(), anyInt(), any(DownstreamAuthHeaders.class)))
                .thenReturn(ResponseEntity.ok("[]"));
        when(gatewayService.userExposure(eq("u1"), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("exp"));
        when(gatewayService.totalExposure(any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("total"));
        when(gatewayService.walletBalance(eq("u1"), any(DownstreamAuthHeaders.class))).thenReturn(ResponseEntity.ok("100"));
