package com.sportsbetting.oddsservice.integration;
...
/**
 * Publishes a pending odds outbox row via the polling dispatcher against real Kafka and Postgres
 * (Testcontainers), mirroring the betting-service outbox IT pattern.
 */
@Testcontainers
@SpringBootTest(
...
class OutboxRelayIT {
    ...
    static void properties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.jpa.properties.hibernate.default_schema", () -> "odds");
    ...
    void outboxDispatcherPublishesToKafkaAndMarksEventAsPublished() {
        String json = """
                {"eventId":"event-it-1","selection":"HOME","odds":2.42,"updatedAt":"2026-01-15T10:00:00Z"}
                """.trim();
        OutboxEventEntity row = new OutboxEventEntity();
        row.setId(UUID.randomUUID());
        row.setEventType(OddsUpdatedEvent.EVENT_TYPE);
        row.setPayload(json);
        ...
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    List<ConsumerRecord<String, String>> records = consumeFrom(OddsUpdatedEvent.EVENT_TYPE);
    ...
    private List<ConsumerRecord<String, String>> consumeFrom(String topic) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "it-odds-outbox-" + UUID.randomUUID());
