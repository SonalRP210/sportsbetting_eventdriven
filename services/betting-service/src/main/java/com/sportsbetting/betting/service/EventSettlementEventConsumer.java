...
public class EventSettlementEventConsumer {

    private final BettingService bettingService;
    private final ProcessedEventRepository processedEventRepository;
    private final ObjectMapper objectMapper;

    public EventSettlementEventConsumer(
    ...
    @KafkaListener(
            topics = "${app.kafka.topic.eventSettled:settlement.event.settled.v1}",
            groupId = "${app.kafka.group.betting.settlement:betting-service-settlement}"
    )
    @Transactional
    public void onEventSettled(String payload, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
        processOnce(topic, payload, () -> {
            ...
            bettingService.applyEventSettlement(asString(data.get("eventId")), asString(data.get("winningSelection")));
        });
    }
